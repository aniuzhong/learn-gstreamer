...
typedef struct _CustomData {
    ...
    gulong slider_update_signal_ld; /* signal ID for the slider update signal */
...
{
    std::cout << "id:" << std::this_thread::get_id() << ',' << "play_cb..\n";
...
{
    std::cout << "id:" << std::this_thread::get_id() << ',' << "pause_cb..\n";
...
{
    std::cout << "id:" << std::this_thread::get_id() << ',' << "stop_cb..\n";
    gst_element_set_state(data->playbin, GST_STATE_READY);
}

/* This function is called when the main window is closed */
static void delete_event_cb(GtkButton *button, CustomData *data)
{
    std::cout << "id:" << std::this_thread::get_id() << ',' << "delete_event_cb..\n";
    stop_cb(NULL, data);
    gtk_main_quit();
}
...
static void slider_cb(GtkRange *range, CustomData *data)
{
    ...
    std::cout << "id:" << std::this_thread::get_id() << ',' << "slider_cb," << value << '\n';
...
static void create_ui(CustomData *data)
{
    ...
    data->slider_update_signal_ld = g_signal_connect(G_OBJECT(data->slider), "value-changed",
    ...
    g_print("\n");
    std::cout << gdk_x11_window_get_xid(gtk_widget_get_window(main_window)) << '\n';    
    std::cout << gdk_x11_window_get_xid(gtk_widget_get_window(main_box)) << '\n';    
    std::cout << gdk_x11_window_get_xid(gtk_widget_get_window(play_button)) << '\n';    
}

/* This function is called periodically to refresh the GUI */
static gboolean refresh_ui(CustomData *data)
{
    std::cout << "id:" << std::this_thread::get_id() << ',' << "refresh_ui..\n";
    gint64 current = -1;
    ...
    if (gst_element_query_position(data->playbin, GST_FORMAT_TIME, &current)) {
        /* Block the "value-changed" signal, so the slider_cb funciton is not called
         * (which would trigger a seek the user has not requeseted) */
        g_signal_handler_block(data->slider, data->slider_update_signal_ld);
        /* Set the position of the slider to the current pipeline position, in SECONDS */
        gtk_range_set_value(GTK_RANGE(data->slider), (gdouble)current / GST_SECOND);
        /* Re-enable the signal */
        g_signal_handler_unblock(data->slider, data->slider_update_signal_ld);
    }
    return TRUE;
}
...
static void tags_cb(GstElement *playbin, gint stream, CustomData *data)
{
    ...
    std::cout << "id:" << std::this_thread::get_id() << ',' << "tags_cb..\n";
...
{
    std::cout << "id:" << std::this_thread::get_id() << ',' << "error_cb..\n";
    GError *err;
    gchar *debug_info;
...
{
    std::cout << "id:" << std::this_thread::get_id() << ',' << " eos_cb,";
...
{
    std::cout << "id:" << std::this_thread::get_id() << ',' << " state_changed_cb..\n";
...
{
    std::cout << "id:" << std::this_thread::get_id() << ',' << "analyze_streams..\n";
    gint i;
    GstTagList *tags;
    gchar *str, *total_str;
    guint rate;
...
{
    std::cout << "id:" << std::this_thread::get_id() << ',' << "application_cb..\n";
...
int main(int argc, char *argv[])
{
    ...
    /* Set the URI to play */
    g_object_set(data.playbin, "uri", "file:///home/aniu/Videos/Forrest_Gump_IMAX.mp4", NULL);
    ...
    /* Register a function that GLib call every second */
